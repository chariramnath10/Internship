...
export default function TaskApp({
  ...
  const categories = [
    ...new Set(tasks.map((task) => task.category || "General").filter(Boolean)),
  ];
  ...
  const searchFilteredTasks = categoryFilteredTasks.filter((task) => {
    ...
    return (
      ...
      (task.category || "General").toLowerCase().includes(search) ||
  ...
  const dueDateValue = (dueDate?: string | number) => {
  ...
  const sortedTasks = [...searchFilteredTasks].sort((a, b) => {
    switch (sortOrder) {
      ...
      case "due-date": {
        const aDate = dueDateValue(a.dueDate);

        const bDate = dueDateValue(b.dueDate);
